class Task < ApplicationRecord
  ...
  # scope :with_tag, -> (search_content) { where(tagging: {content: search_content}) }
  ...
  def tag_items=(contents)
    self.tags = contents.map{|item|
      Tag.where(content: item.strip).first_or_create! unless item.blank?}.compact!
  end
end
